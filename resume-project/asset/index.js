...
function init() {
  var validUser = JSON.parse(localStorage.getItem("isloggedin"));
  ...
  if (pathname === "/index.html" && validUser) {
    ...
    up1 = {
      name: "ravi0609",
      password: btoa("ravi@123"),
    };
    a.push(up1);
    // console.log(a)
    localStorage.setItem("all_users", JSON.stringify(a));
  } else if (pathname === "/resume.html" && validUser) {
    var jsondata = JSON.parse(sessionStorage.getItem("resumedata"));
    if (!jsondata) {
      getJsonData();
    }
    fetchall();
...
function Login() {
  ...
  if (username != "" && password != "") {
    var validUser = JSON.parse(localStorage.getItem("all_users"));
    ...
    if (isvalid) {
      ...
      b.push(up2);
      // console.log(a)
      localStorage.setItem("isloggedin", JSON.stringify(b));
      location.href = "/resume.html";
    } else {
...
function fetchall() {
  jsondata = JSON.parse(sessionStorage.getItem("resumedata"));
